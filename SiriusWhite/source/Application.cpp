...
HWND Application::createWindow()
{
    ...
    m_handleWindow = ::CreateWindowW(m_wc.lpszClassName, L"Sirius White", WS_OVERLAPPEDWINDOW, 100, 100, 1000, 800, nullptr, nullptr, m_wc.hInstance, nullptr);
...
void Application::Run()
{
    ...
    ImGui_ImplWin32_Init(m_handleWindow);

    static auto* pSrvHeapAlloc = ptrD3D12Context->GetSrvHeapAlloc();

    ImGui_ImplDX12_InitInfo init_info = {};
    init_info.Device = ptrD3D12Context->GetID3D12Device();
    init_info.CommandQueue = ptrD3D12Context->GetID3D12CommandQueue();
    ...
    while (!done)
    {
        ...
        if (ptrD3D12Context->isSwapChainOccluded() && ptrD3D12Context->GetIDXGISwapChain3()->Present(0, DXGI_PRESENT_TEST) == DXGI_STATUS_OCCLUDED)
        {
            ::Sleep(10);
            continue;
        }
...
LRESULT CALLBACK Application::WindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_SIZE:
        if (ptrD3D12Context->GetID3D12Device() != nullptr && wParam != SIZE_MINIMIZED)
        {
            ptrD3D12Context->WaitForLastSubmittedFrame();
            ptrD3D12Context->CleanupRenderTarget();
            HRESULT result = ptrD3D12Context->GetIDXGISwapChain3()->ResizeBuffers(0, (UINT)LOWORD(lParam), (UINT)HIWORD(lParam), DXGI_FORMAT_UNKNOWN, DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT);
