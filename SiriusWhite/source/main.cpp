...
#include "components/window/Win32Window.hpp"
#include "components/window/WindowTool.hpp"
#include "components/storage/CustomStorage.hpp"

#include <memory>
#include <iostream>

// Forward declarations of helper functions
std::unique_ptr<D3D12Context> ptrD3D12Context = std::make_unique<D3D12Context>();
...
int main(int, char**)
{
    ...
    try
    {
        ...
        //wc = WNDCLASSEXW{ sizeof(wc), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(nullptr), nullptr, nullptr, nullptr, nullptr, L"ImGui Example", nullptr }; old impl
        ...
        if (!::RegisterClassExW(&wc))
             throw ui::exception("Main window class register failed", ::GetLastError());

        hwnd = ::CreateWindowW(wc.lpszClassName, L"Sirius White", WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, nullptr, nullptr, wc.hInstance, nullptr);

        auto ptrWindowStorage = std::make_unique<CustomStorage<HWND, Win32::WindowTool>>();
        
        auto* ptrWindowTool = new Win32::WindowTool(L"label", L"GOOGLE", 100, 100, 500, 500);
        ptrWindowTool->show();
        ptrWindowStorage->insert({ ptrWindowTool->key(), ptrWindowTool });

        ptrD3D12Context->initialize(hwnd);
        ...
        init_info.Device = ptrD3D12Context->Get<ID3D12Device>();
        init_info.CommandQueue = ptrD3D12Context->Get<ID3D12CommandQueue>();
        init_info.NumFramesInFlight = APP_NUM_FRAMES_IN_FLIGHT;
        init_info.RTVFormat = DXGI_FORMAT_R8G8B8A8_UNORM;
        // Allocating SRV descriptors (for textures) is up to the application, so we provide callbacks.
        // (current version of the backend will only allocate one descriptor, future versions will need to allocate more)
        init_info.SrvDescriptorHeap = ptrD3D12Context->GetSrvHeap();
        init_info.SrvDescriptorAllocFn = [](ImGui_ImplDX12_InitInfo*, D3D12_CPU_DESCRIPTOR_HANDLE* out_cpu_handle, D3D12_GPU_DESCRIPTOR_HANDLE* out_gpu_handle) { return ptrD3D12Context->GetSrvHeapAlloc()->Alloc(out_cpu_handle, out_gpu_handle); };
        init_info.SrvDescriptorFreeFn = [](ImGui_ImplDX12_InitInfo*, D3D12_CPU_DESCRIPTOR_HANDLE cpu_handle, D3D12_GPU_DESCRIPTOR_HANDLE gpu_handle) { return ptrD3D12Context->GetSrvHeapAlloc()->Free(cpu_handle, gpu_handle); };
        ...
        // io.Fonts->AddFontFromFileTTF("c:\\Windows\\Fonts\\Lato.ttf", 14.0f);
        ...
        while (!done)
        {
            ...
            if (ptrD3D12Context->isSwapChainOccluded() && ptrD3D12Context->Get<IDXGISwapChain3>()->Present(0, DXGI_PRESENT_TEST) == DXGI_STATUS_OCCLUDED)
            {
                ::Sleep(10);
                continue;
            }
            ptrD3D12Context->setSwapChainOccluded(false);
            ...
            if (isPlotWindow)
            {
                ...
                int lengthData = 218;
                float lineX[] = { 1546819200, 1547424000 }; // line
                float lineY[] = { 1400 , 1450 };            // line
                ...
                // Here is new implementation of CandleStick plotting
                ImGui::Begin("Plot Window", &isPlotWindow);
               
                if (ImPlot::BeginCandlestickChartPlot("Candlestick Chart", ImVec2(-1, 0), ImPlotFlags_NoTitle)) { // here is possible 
                    ImPlot::SetupAxes(nullptr, nullptr, 0, ImPlotAxisFlags_AutoFit | ImPlotAxisFlags_RangeFit);
                    //ImPlot::PlotLine("##angle", lineX, lineY, 2);
                    uint64_t timeMin = 1546300800;
                    uint64_t timeMax = 1571961600;
                    uint64_t priceMin = 1250;
                    uint64_t priceMax = 1600;
                    ImPlot::SetupAxesLimits(timeMin, timeMax, priceMin, priceMax);
                    ImPlot::SetupAxisScale(ImAxis_X1, ImPlotScale_Time);
                    ImPlot::SetupAxisLimitsConstraints(ImAxis_X1, timeMin, timeMax);
                    ImPlot::SetupAxisZoomConstraints(ImAxis_X1, 60 * 60 * 24 * 14, timeMax - timeMin);
                    ImPlot::SetupAxisFormat(ImAxis_Y1, "$%.0f");
                    MyImPlot::PlotCandlestick("GOOGL", dates, opens, closes, lows, highs, lengthData, tooltip, 0.25f, bullCol, bearCol);
                    ImPlot::EndPlot();
                }
                ImGui::End();
            }
            ...
            ptrD3D12Context->Rendering(clear_color);
        }

        ptrD3D12Context->WaitForLastSubmittedFrame();
...
LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_SIZE:
        if (ptrD3D12Context->Get<ID3D12Device>() != nullptr && wParam != SIZE_MINIMIZED)
        {
            ptrD3D12Context->WaitForLastSubmittedFrame();
            ptrD3D12Context->CleanupRenderTarget();
            HRESULT result = ptrD3D12Context->Get<IDXGISwapChain3>()->ResizeBuffers(0, (UINT)LOWORD(lParam), (UINT)HIWORD(lParam), DXGI_FORMAT_UNKNOWN, DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT);
            assert(SUCCEEDED(result) && "Failed to resize swapchain.");
            ptrD3D12Context->CreateRenderTarget();
        }
        return 0;
    case WM_SYSCOMMAND:
